...
RUN pip install --no-cache -r requirements.txt coremltools onnx onnxruntime onnxruntime-gpu flask gsutil notebook wandb>=0.12.2
...
RUN pip install --no-cache torch==1.8.0+cu111 torchvision==0.9.0+cu111 torchaudio==0.8.0 -f https://download.pytorch.org/whl/torch_stable.html
